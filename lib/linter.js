...
const DMNLINT_PLUGIN_PREFIX = 'dmnlint-plugin-';
...
Linter.prototype.resolveRules = function(config) {
  ...
  return this.resolveConfiguredRules(config).then((rulesConfig) => {
    ...
    const enabledRules = parsedRules;
...
Linter.prototype.resolveConfiguredRules = function(config) {
  ...
  return Promise.all(
    ...
    const overrideRules = this.normalizeConfig(config, DMNLINT).rules;
 ...
 *
 * @param {ModdleElement} moddleRoot
 * @param {Object} [config] the dmnlint configuration to use
 *
...
Linter.prototype.parseRuleName = function(name, localPackage = DMNLINT) {

  /**
   * We recognize the following rule name patterns:
   *
   * {RULE_NAME} => PKG = LINTER_PKG
   * dmnlint/{RULE_NAME} => PKG = LINTER_PKG
   * {PACKAGE_SHORTCUT}/{RULE_NAME} => PKG = 'dmnlint-plugin-{PACKAGE_SHORTCUT}'
   * dmnlint-plugin-{PACKAGE_SHORTCUT}/{RULE_NAME} => PKG = 'dmnlint-plugin-{PACKAGE_SHORTCUT}'
   * @scope/{PACKAGE_SHORTCUT}/{RULE_NAME} => PKG = '@scope/dmnlint-plugin-{PACKAGE_SHORTCUT}'
   * @scope/dmnlint-plugin-{PACKAGE_SHORTCUT}/{RULE_NAME} => PKG = '@scope/dmnlint-plugin-{PACKAGE_SHORTCUT}'
   */
...
Linter.prototype.parseConfigName = function(name) {
   ...
   *
   * dmnlint:{CONFIG_NAME} => PKG = LINTER_PKG
   * plugin:{PACKAGE_SHORTCUT}/{CONFIG_NAME} => PKG = 'dmnlint-plugin-{PACKAGE_SHORTCUT}'
   * plugin:dmnlint-plugin-{PACKAGE_SHORTCUT}/{CONFIG_NAME} => PKG = 'dmnlint-plugin-{PACKAGE_SHORTCUT}'
   * plugin:@scope/{PACKAGE_SHORTCUT}/{CONFIG_NAME} => PKG = '@scope/dmnlint-plugin-{PACKAGE_SHORTCUT}'
   * plugin:@scope/dmnlint-plugin-{PACKAGE_SHORTCUT}/{CONFIG_NAME} => PKG = '@scope/dmnlint-plugin-{PACKAGE_SHORTCUT}'
   */

  const match = /^(?:(?:plugin:(?:(@[^/]+)\/)?([^@]{1}[^/]*)\/)|dmnlint:)([^/]+)$/.exec(name);
...
Linter.prototype.getSimplePackageName = function(name) {
   ...
   * dmnlint-plugin-{PACKAGE_SHORTCUT}' => PKG = PACKAGE_SHORTCUT
   * @scope/{PACKAGE_SHORTCUT} => PKG = '@scope/{PACKAGE_SHORTCUT}'
   * @scope/dmnlint-plugin-{PACKAGE_SHORTCUT}' => PKG = '@scope/PACKAGE_SHORTCUT'
   */
...
function prefixPackage(pkg) {
  ...
  if (pkg.startsWith(DMNLINT_PLUGIN_PREFIX)) {
    return pkg;
  }

  return `dmnlint-plugin-${pkg}`;
}
...
function unprefixPackage(pkg) {
  ...
  if (pkg.startsWith(DMNLINT_PLUGIN_PREFIX)) {
    return pkg.substring(DMNLINT_PLUGIN_PREFIX.length);
  }
